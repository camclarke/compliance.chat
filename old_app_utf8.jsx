    ...
    try {
      ...
      if (error.response?.status === 429) {
        ...
        const quotaMessage = {
          id: Date.now() + 1,
          role: 'assistant',
          content: "ΓÜá∩╕Å You've reached your daily token limit. Please upgrade your subscription to continue using compliance.chat.",
          timestamp: new Date().toISOString()
        };
              ...
              <div className="token-budget-bar">
                <div className="token-budget-info">
                  ...
                  <span className="token-budget-count">
                    {tokensRemaining < 0 ? 'Γê₧' : tokensRemaining.toLocaleString()} / {tokensLimit < 0 ? 'Γê₧' : tokensLimit.toLocaleString()}
                  </span>
                </div>
